...
# =============================================
# --- APPROVAL SYSTEM DATA STRUCTURES ---
approved_users = set()      # Approved users ka set
pending_users = {}          # {user_id: {name, username, bio, photo_file_id}}
# =============================================
...
ADMIN_COMMAND_BUTTONS_LAYOUT_USER_SPEC = [
    ...
    ["✅ Pending Approvals", "📞 Contact Owner"]  # Approval button added
]
...
def is_user_approved(user_id):
    """Check karo ki user approved hai ya nahi"""
    if user_id == OWNER_ID or user_id in admin_ids:
        return True
    return user_id in approved_users

def approve_user_db(user_id):
    """User ko approve karo aur DB mein save karo"""
    ...
    with DB_LOCK:
        ...
        try:
            ...
            conn.commit()
            logger.info(f"User {user_id} approved and saved to DB.")
        except sqlite3.Error as e:
            logger.error(f"DB error approving user {user_id}: {e}")
        finally:
            conn.close()

def reject_user_db(user_id):
    """User ko reject karo - approved list se hata do agar hai"""
    ...
    with DB_LOCK:
        ...
        try:
            ...
            logger.error(f"DB error rejecting user {user_id}: {e}")
        finally:
            conn.close()

def send_approval_request(user_id, user_name, user_username, user_bio, photo_file_id):
    """Owner aur admins ko approval request bhejo"""
    ...
    markup.row(
        types.InlineKeyboardButton(f"✅ Approve", callback_data=f'approve_user_{user_id}'),
        types.InlineKeyboardButton(f"❌ Reject", callback_data=f'reject_user_{user_id}')
    )
    ...
    request_text = (
        ...
        f"🆔 User ID: `{user_id}`\n"
        f"📝 Bio: {user_bio}\n\n"
        f"Approve karo ya reject karo:"
    )

    # Sabhi admins ko request bhejo
    for admin_id in admin_ids:
        try:
            if photo_file_id:
                bot.send_photo(admin_id, photo_file_id, caption=request_text, parse_mode='Markdown', reply_markup=markup)
            else:
                bot.send_message(admin_id, request_text, parse_mode='Markdown', reply_markup=markup)
        except Exception as e:
            logger.error(f"Could not send approval request to admin {admin_id}: {e}")

def _logic_pending_approvals(message):
    """Pending approvals dikhao - Admin only"""
    if message.from_user.id not in admin_ids:
        bot.reply_to(message, "⚠️ Admin permissions required.")
        return

    if not pending_users:
        bot.reply_to(message, "✅ Koi pending approval nahi hai.")
        return

    markup = types.InlineKeyboardMarkup(row_width=2)
    msg_text = f"⏳ *Pending Approvals ({len(pending_users)}):*\n\n"

    for uid, info in list(pending_users.items()):
        msg_text += f"👤 {info['name']} (@{info['username'] or 'N/A'}) - `{uid}`\n"
        markup.row(
            types.InlineKeyboardButton(f"✅ {info['name']}", callback_data=f'approve_user_{uid}'),
            types.InlineKeyboardButton(f"❌ Reject", callback_data=f'reject_user_{uid}')
        )
        ...
        try:
            ...
            if not is_running:
                if 'log_file' in script_info and hasattr(script_info['log_file'], 'close') and not script_info['log_file'].closed:
                    ...
                    except Exception as log_e: logger.error(f"Error closing log file: {log_e}")
...
def run_js_script(script_path, script_owner_id, user_folder, file_name, message_obj_for_reply, attempt=1):
    ...
    try:
        ...
        try:
            ...
            if log_file and not log_file.closed: log_file.close()
            bot.reply_to(message_obj_for_reply, f"❌ Error starting JS script '{file_name}': {str(e)}")
            if process and process.poll() is None:
                kill_process_tree({'process': process, 'log_file': log_file, 'script_key': script_key})
            if script_key in bot_scripts: del bot_scripts[script_key]
    except Exception as e:
...
def remove_subscription_db(user_id):
    with DB_LOCK:
        ...
        except sqlite3.Error as e: logger.error(f"❌ SQLite error removing subscription: {e}")
        finally: conn.close()
...
def add_admin_db(admin_id):
    with DB_LOCK:
        ...
        try:
            ...
            approved_users.add(admin_id)  # Admin automatically approved
...
def create_main_menu_inline(user_id):
    ...
    if user_id in admin_ids:
        admin_buttons = [
            ...
            types.InlineKeyboardButton('✅ Pending Approvals', callback_data='pending_approvals'),  # ADDED
        ]
        ...
        markup.add(admin_buttons[6])  # Pending Approvals button
...
def send_log_file(message, log_path, log_filename):
    try:
        ...
        if file_size > 50 * 1024 * 1024:
            bot.reply_to(message, f"❌ Log file too large ({file_size/1024/1024:.1f} MB).")
            return
...
def _logic_send_welcome(message):
    ...
    if not is_user_approved(user_id):
        # Pehli baar aa raha hai ya pending hai
        if user_id not in pending_users:
            # Approval request bhejo
            send_approval_request(user_id, user_name, user_username, user_bio, photo_file_id)
            bot.send_message(
                chat_id,
                f"👋 Welcome, {user_name}!\n\n"
                f"⏳ *Aapki access request send ho gayi hai.*\n"
                f"Admin approval ke baad aap bot use kar sakte hain.\n\n"
                f"🔔 Approved hone par notification milega.",
                parse_mode='Markdown'
            )
            logger.info(f"Approval request sent for new user {user_id} (@{user_username})")
        else:
            # Already pending hai
            bot.send_message(
                chat_id,
                f"⏳ *Aapki request already pending hai.*\n"
                f"Admin review kar raha hai. Please wait.\n\n"
                f"🆔 Your ID: `{user_id}`",
                parse_mode='Markdown'
            )
        return
    ...
    # Approved user ke liye normal welcome
    if user_id not in active_users:
        add_active_user(user_id)
        try:
            owner_notification = (f"🎉 New approved user active!\n👤 Name: {user_name}\n"
...
def _logic_upload_file(message):
    ...
    # APPROVAL CHECK
    if not is_user_approved(user_id):
        bot.reply_to(message, "⏳ Aapki request pending hai. Admin approval ka wait karo.")
        return
    # =============================================
    if bot_locked and user_id not in admin_ids:
...
def _logic_statistics(message):
    ...
    running_bots_count = 0; user_running_bots = 0
    ...
    if user_id in admin_ids:
        # =============================================
        # Show pending approvals in stats for admin
            ...
            if not is_bot_running(target_user_id, file_name):
                ...
                if os.path.exists(file_path):
                    try:
                        ...
                        error_files_details.append(f"`{file_name}` (User {target_user_id}) - Error")
                        skipped_files += 1
                else:
                    error_files_details.append(f"`{file_name}` (User {target_user_id}) - File not found")
                    skipped_files += 1
    summary_msg = (f"✅ All Users' Scripts - Done:\n\n▶️ Started: {started_count} scripts.\n"
                   f"👥 Users: {attempted_users}.\n")
    if skipped_files > 0:
        summary_msg += f"⚠️ Skipped/Errors: {skipped_files}\n"
...
BUTTON_TEXT_TO_LOGIC = {
    ...
    "✅ Pending Approvals": _logic_pending_approvals,  # ADDED
}
...
@bot.message_handler(commands=['pendingapprovals'])  # ADDED
...
def handle_file_upload_doc(message):
    ...
    try:
        ...
        else: bot.reply_to(message, f"❌ Telegram API Error: {str(e)}.")
    except Exception as e:
...
def handle_callbacks(call):
    ...
    # =============================================
    # APPROVAL CALLBACKS - pehle check karo
    if data.startswith('approve_user_'):
        if user_id not in admin_ids:
            bot.answer_callback_query(call.id, "⚠️ Admin only.", show_alert=True)
            return
        approve_user_callback(call)
        return
    if data.startswith('reject_user_'):
        if user_id not in admin_ids:
            bot.answer_callback_query(call.id, "⚠️ Admin only.", show_alert=True)
            return
        reject_user_callback(call)
        return
    ...
    # =============================================
    # Non-approval callbacks ke liye approval check
    if not is_user_approved(user_id) and data not in ['back_to_main', 'speed', 'stats']:
        bot.answer_callback_query(call.id, "⏳ Aapki request pending hai.", show_alert=True)
        return
    # =============================================
...
def approve_user_callback(call):
    ...
    try:
        target_user_id = int(call.data.replace('approve_user_', ''))
        admin_who_approved = call.from_user.id

        if is_user_approved(target_user_id):
            bot.answer_callback_query(call.id, f"✅ User {target_user_id} already approved.", show_alert=True)
            return

        approve_user_db(target_user_id)
        pending_users.pop(target_user_id, None)

        # Approval message edit karo
        try:
            original_text = call.message.caption or call.message.text or ""
            new_text = original_text + f"\n\n✅ *Approved by admin {admin_who_approved}*"
            if call.message.caption:
                bot.edit_message_caption(new_text, call.message.chat.id, call.message.message_id,
        ...
        bot.answer_callback_query(call.id, f"✅ User {target_user_id} approved!", show_alert=True)
        logger.info(f"User {target_user_id} approved by admin {admin_who_approved}")

        # User ko notify karo
        try:
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton('🚀 Start Bot', callback_data='back_to_main'))
            bot.send_message(
                target_user_id,
                "🎉 *Congratulations! Aapki request approve ho gayi!*\n\n"
                "Ab aap bot use kar sakte hain. /start karke shuru karo.",
                parse_mode='Markdown',
                reply_markup=markup
            )
        except Exception as e:
            logger.error(f"Could not notify approved user {target_user_id}: {e}")

        # Baaki admins ko bhi notify karo
        for aid in admin_ids:
            if aid != admin_who_approved:
                try:
                    bot.send_message(aid, f"✅ User `{target_user_id}` was approved by admin `{admin_who_approved}`.", parse_mode='Markdown')
                except Exception: pass

    except Exception as e:
        logger.error(f"Error in approve_user_callback: {e}", exc_info=True)
        bot.answer_callback_query(call.id, "Error approving user.", show_alert=True)

def reject_user_callback(call):
    """User ko reject karo"""
    try:
        target_user_id = int(call.data.replace('reject_user_', ''))
        admin_who_rejected = call.from_user.id

        reject_user_db(target_user_id)

        # Message edit karo
        try:
            original_text = call.message.caption or call.message.text or ""
            new_text = original_text + f"\n\n❌ *Rejected by admin {admin_who_rejected}*"
            if call.message.caption:
                bot.edit_message_caption(new_text, call.message.chat.id, call.message.message_id,
                                        parse_mode='Markdown', reply_markup=None)
            else:
        ...
        bot.answer_callback_query(call.id, f"❌ User {target_user_id} rejected.", show_alert=True)
        logger.info(f"User {target_user_id} rejected by admin {admin_who_rejected}")

        # User ko notify karo
        try:
            bot.send_message(
                target_user_id,
                "❌ *Aapki bot access request reject ho gayi hai.*\n\n"
                "Agar aapko lagta hai yeh galti se hua hai, toh owner se contact karo.",
                parse_mode='Markdown'
            )
        except Exception as e:
            logger.error(f"Could not notify rejected user {target_user_id}: {e}")

    except Exception as e:
        logger.error(f"Error in reject_user_callback: {e}", exc_info=True)
        bot.answer_callback_query(call.id, "Error rejecting user.", show_alert=True)
...
def check_files_callback(call):
    ...
    except Exception as e: logger.error(f"Error editing file list: {e}", exc_info=True)
...
def start_bot_callback(call):
    try:
        ...
        if file_type == 'py':
            ...
            bot.send_message(chat_id_for_reply, f"❌ Unknown file type."); return
        time.sleep(1.5)
...
def delete_bot_callback(call):
    try:
        ...
        try:
            bot.edit_message_text(
                f"🗑️ Record `{file_name}` and {deleted_str} deleted!",
...
def stats_callback(call):
    ...
    except Exception as e: logger.error(f"Error updating menu after stats: {e}")
...
def process_remove_admin_id(message):
    ...
    try:
        ...
        else: bot.reply_to(message, f"❌ Failed to remove admin.")
    except ValueError:
        bot.reply_to(message, "⚠️ Invalid ID.")
...
def process_add_subscription_details(message):
    ...
    try:
        ...
        if sub_user_id <= 0 or days <= 0: raise ValueError("Values must be positive")
        ...
        try: bot.send_message(sub_user_id, f"🎉 Subscription activated! Expires: {new_expiry:%Y-%m-%d}.")
        except Exception: pass
...
def process_remove_subscription_id(message):
    ...
    try:
        ...
        if sub_user_id_remove not in user_subscriptions:
            bot.reply_to(message, f"⚠️ User `{sub_user_id_remove}` has no active sub."); return
        remove_subscription_db(sub_user_id_remove)
        bot.reply_to(message, f"✅ Sub for `{sub_user_id_remove}` removed.")
        try: bot.send_message(sub_user_id_remove, "ℹ️ Your subscription removed.")
        except Exception: pass
    except ValueError:
        bot.reply_to(message, "⚠️ Invalid ID.")
        msg = bot.send_message(message.chat.id, "💳 Enter User ID, or /cancel.")
        ...
        if sub_user_id_check in user_subscriptions:
            ...
            if expiry_dt:
                if expiry_dt > datetime.now():
                    ...
                    bot.reply_to(message, f"✅ User `{sub_user_id_check}` active.\nExpires: {expiry_dt:%Y-%m-%d} ({days_left} days left).")
                else:
                    bot.reply_to(message, f"⚠️ User `{sub_user_id_check}` expired.")
...
if __name__ == '__main__':
    logger.info("="*40 + "\n🤖 Bot Starting Up (With Approval System)...\n" + "="*40)
    keep_alive()
